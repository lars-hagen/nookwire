...
#
# Env overrides:
#   NOOKWIRE_VERSION (alias: NOOKWIRE_SSH_VERSION)     pin a release tag (e.g. 2.3.1) instead of the default.
...
VERSION=${NOOKWIRE_VERSION:-${NOOKWIRE_SSH_VERSION:-2.3.1}}
...
if [ -z "$PACKAGE" ]; then
  ...
  # …/nookwire@main@v2.3.1, which uv cannot parse.
  case "${BASE#*://}" in
    *@*)
      PACKAGE="$BASE"
      ;;
